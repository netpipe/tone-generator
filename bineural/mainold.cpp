...
public:
    ToneGeneratorWidget(QWidget *parent = nullptr) : QWidget(parent), phase(0), currentWave(SINE), playing(false), frequency(440), frequency2(440) {
...
private slots:
    ...
    void onBinauralButtonClicked() {
        frequency = frequencyInput->text().toInt(); // Update frequency when button is clicked
        frequency2 = frequency + 10; // Slightly different frequency for binaural beats
